    ...
    <el-card class="box-card login-card" id="login_card">
      ...
      <el-form :model="form" :rules="rule" class="form" label-width="0px" ref="form">
        <el-form-item prop="username">
          <el-input v-model="form.username" :disabled="requesting" autofocus
                    :placeholder="$transform('login.placeholder.account')" class="login-input"
                    @keyup.enter.native="login(vm, 'form', form, url, vm.language)">
            <i class="el-input__icon el-icon-user" slot="prefix"></i>
          </el-input>
        </el-form-item>
        <el-form-item prop="password">
          <el-input v-model="form.password" :disabled="requesting"
      ...
      <el-button :loading="requesting" @click="login(vm, 'form', form, url, vm.language)" class="login-button"
