...
export default {
  ...
  props: {
    ...
    admin: { type: Boolean, default: false }, // 是否去请求大后台地址
    value: { type: String, default: 'id' }
  },
  methods: {
    init() {
      // 再次编辑时没有该选项则清空数据
      let flag = this.pool.some(item => {
        return item[this.value] === this.select_id
      })
      if (flag) this.id = this.$deepCopy(this.select_id)
      else this.clear()
    },
    getList(post = {}) {
      let that = this
      let base = ''
      if (that.admin) base = this.$deepCopy(api)
      that.$apiPost(that.url, post, base).then((res) => {
  ...
  watch: {
    select_id(val) {
      // 再次编辑时没有该选项则清空数据
      let flag = this.pool.some(item => {
        return item[this.value] === val
      })
      if (flag) this.id = this.$deepCopy(val)
      else this.clear()
    }
  }
}
